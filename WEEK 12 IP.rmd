...
```{r}
num_df <- ad[, c("Age", "Area Income", "Daily Internet Usage")]
cat_df <- ad[, c("Ad Topic Line", "City", "Country")]
```
...
WEEK 13 IP ON SUPERVISED LEARNING MODELS

# Change the specific column name to avoid spaces
```{r}
colnames(ad)[colnames(ad) == "Clicked_on_Ad"] <- "clicked_on_Ad"
head(ad)
```
...
```
```{r}
# Training the model for Decesion trees
...
```
Multiple Regression model(linear)

```{r}
multiple_lm <- lm(clicked_on_ad ~ ., ad)
